defmodule VideoSyncWeb.RoomLive.FormComponent do
  use VideoSyncWeb, :live_component
  ...
  def update(%{room: room} = assigns, socket) do
    changeset = Player.change_room(room)
  ...
  def handle_event("validate", %{"room" => room_params}, socket) do
    changeset =
      socket.assigns.room
      |> Player.change_room(room_params)
  ...
  defp save_room(socket, :edit, room_params) do
    case Player.update_room(socket.assigns.room, room_params) do
      {:ok, room} ->
  ...
  defp save_room(socket, :new, room_params) do
    case Player.create_room(room_params) do
      {:ok, room} ->
        notify_parent({:saved, room})
